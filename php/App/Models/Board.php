...
class Board
{
    ...
    */
    ...
    public function checkWin($jugador): bool { //Comprova si hi ha un guanyador (Se puede tener diferente)
        ...
        for ($i = 1; $i <= $files; $i++) {
            for ($j = 1; $j <= $columnes - 3; $j++) {
                if ($this->slots[$i][$j] == $jugador && $this->slots[$i][$j + 1] == $jugador &&
        ...
        for ($i = 1; $i <= $files - 3; $i++) {
            for ($j = 1; $j <= $columnes; $j++) {
                if ($this->slots[$i][$j] == $jugador && $this->slots[$i + 1][$j] == $jugador &&
        ...
        for ($i = 3; $i < $files; $i++) {
            for ($j = 1; $j < $columnes - 3; $j++) {
                if ($this->slots[$i][$j] == $jugador && $this->slots[$i - 1][$j + 1] == $jugador &&
    ...
    public function isValidMove(int $column): bool { //Comprova si el moviment és vàlid (Ccomprueba si el jugador está intentando jugar en una columna llena)
